...
namespace LeventureDesign.Admin.UsermanagerFrm
{
    public partial class UserAdd : Form
    {
        ...
        {
            if (dataGridView1.Rows[0].Cells[0].Value.ToString().Trim() == "") //假如未输入数据
            {
                MessageBox.Show("输入姓名不能为空！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (dataGridView1.Rows[0].Cells[1].Value.ToString().Trim() == "")
            {
                MessageBox.Show("输入账号不能为空！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (dataGridView1.Rows[0].Cells[2].Value.ToString().Trim() == "")
            {
                MessageBox.Show("输入密码不能为空！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (!(string.Equals(dataGridView1.Rows[0].Cells[3].Value.ToString(), "教师") || string.Equals(dataGridView1.Rows[0].Cells[3].Value.ToString(), "学生") || string.Equals(dataGridView1.Rows[0].Cells[3].Value.ToString(), "管理员")))
            {
                MessageBox.Show("输入权限仅包括 “学生” “教师” 和 “管理员”！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (dataGridView1.Rows[0].Cells[4].Value.ToString().Trim() == "")
            {
                MessageBox.Show("输入性别不能为空！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (dataGridView1.Rows[0].Cells[5].Value.ToString().Trim() == "")
            {
                MessageBox.Show("输入电子邮箱不能为空！", "插入提示", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (dataGridView1.Rows[0].Cells[6].Value.ToString().Trim() == "")
            {
