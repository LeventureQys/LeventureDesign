    ...
    public partial class Class_Change : Form
    {
        ...
        private void btn_Confirm_Click(object sender, EventArgs e) //修改数据库用户
        {
            ...
            else
            {
                ...
                nlevel = dataGridView1.Rows[0].Cells[1].Value.ToString();
                nnum = int.Parse(dataGridView1.Rows[0].Cells[2].Value.ToString());
                nteacherid = int.Parse(dataGridView1.Rows[0].Cells[3].Value.ToString().Trim());
                nnumber = int.Parse(dataGridView1.Rows[0].Cells[4].Value.ToString().Trim());
