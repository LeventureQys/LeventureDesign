...
namespace LeventureDesign
{
    public class UserManagerMent
    {
        ...
        public void user_delete(int uid)//删除userid的账户
        {
            ...
            if (num == 1)
            {
                MessageBox.Show("用户删除成功！", "删除提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("用户删除失败，可能不存在该用户！", "删除提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
        //pwdchange做密码修改
        ...
        public DataSet StuDataSet_Classid(int classid)//通过classid返回一个学生的dataset
        {
            try
            {
                string SearchUser = "select * from tb_user where classid='" + classid + "'";
                return dataoper.GetSqlDataset(SearchUser);
            }
            catch (Exception ex)
            {
        ...
        public DataSet userdataset_getTeacher()
        {
            return dataoper.GetSqlDataset("select * from tb_user where authority=2");
        }
        ...
        public void initUser(int uid) //通过uid初始化这个usermanagerment类
        {
            DataSet User = userdataset_search(uid); // 通过账号查询对应用户的dataset
                                        ...
                                        if(!PublicClass.isNullorEmpty(UserEmail,"请输入注册邮箱",type1))
                                            ...
                                            else
                                            {
                                                ...
                                                else //正式插入一个用户
                                                {
                                                    ...
                                                    PublicClass.showMessage("恭喜您，账号注册成功，请等待管理员审核", "注册成功");
                                                }
                                            }
        }
        ...
        public DataSet classviewdataset_search(String TeacherName) //通过姓名查询到教师的st
        {
            string SearchClass = String.Format("select * from vr_classviewer where name='{0}'", TeacherName);
            DataSet st = new DataSet();
        ...
        public DataSet classviewdataset_search(int classid)
        {
            ...
            string SearchClass = String.Format("select * from vr_classviewer where classid='{0}'", classid);
            DataSet st = new DataSet();
            st = dataoper.GetSqlDataset(SearchClass);
            return st;
        }
        ...
        public void class_change(int nclassid, string nlevel, int nnum, int nteacherid, int nnumber)
        {
            try
            {
                ...
                int num = dataoper.ExecSQLResult(change); //运行修改的命令
                if (num == 1)
                {
        ...
        public String returnName_ByClassid(int classid)
        {
            DataSet temp = classtabledataset_search(classid);
            return String.Format("{0}{1}班", PublicClass.getText(temp, 0, 2), PublicClass.getText(temp, 0, 3));
        }

        public String returnLevel_ByClassid(int classid)
        {
            DataSet temp = classtabledataset_search(classid);
            return PublicClass.getText(temp, 0, 2);
        }

        //public void User_isExist()
        //{

        //}
        public String GetLevel(int classid)
        {
            try
            {
                DataSet st = new DataSet();
                //classid = 21;
                st = classtabledataset_search(classid); // 先获得班级的dataset
                return st.Tables[0].Rows[0][3].ToString(); // 取出level
            }catch(Exception ex)
            {
                PublicClass.showMessage(ex.ToString(), "查询年级失败");
                return null;
            }
        }

        public int GetNum_WithClassid(int classid) //通过classid得到班级名称
        {
            try
            {
                DataSet st = new DataSet();
                //classid = 21;
                st = classtabledataset_search(classid); // 先获得班级的dataset
                return int.Parse(st.Tables[0].Rows[0][3].ToString()); // 取出level
            }catch(Exception ex)
            {
                PublicClass.showMessage(ex.ToString(), "班级名称查询失败");
                return -1;
            }
        }

        public DataSet teacher_SearchClass(int userid) // 通过一个老师的账号搜索得到和他相关的所有班级
        {
            string SearchUser = String.Format("select * from tb_class where teachID={0}",userid);
        ...
        public DataSet classtabledataset_searchall()//查找所有班级编号
        {
            string SearchUser ="select * from tb_class" ;
        ...
        public DataSet classView_seachall()
        {
            string SearchUser = "select * from vr_classviewer";
        ...
        public int[] StudentID_Includedby_Classid(int classid) //通过classid获取一个带有对应长度的studentid数组
        {
            String Select = String.Format("select * from tb_user where classid={0}", classid);
            DataSet Student = dataoper.GetSqlDataset(Select); //找到对应班级的dataset
            int[] studentids = new int[Student.Tables[0].Rows.Count];//建立一个长度相当的student数组

            for(int i = 0; i < studentids.Length; i++)
            {
                studentids[i] = PublicClass.getInt(Student, i, 0);
            }

            return studentids;
        }

        public DataSet Student_Includedby_Classid(int classid)//通过classid返回一个对应长度的studentdataset
        {
            String Select = String.Format("select * from tb_user where classid={0}", classid);
            DataSet Student = dataoper.GetSqlDataset(Select); //找到对应班级的dataset
            return Student;
        ...
        public DataSet Classes_ByTeacherid(int teacherid)//通过教师id搜索班级dataset
        {
            return dataoper.GetSqlDataset(String.Format("select * from tb_class where teachID={0}", teacherid));
        }

        public DataSet Classvr_ByTeacherName(String teacher)
        {
            return dataoper.GetSqlDataset(String.Format("select * from vr_classviewer where name='{0}'", teacher));
        }
        
        //通过权限等级查询用户
        public DataSet SearchUser_ByAuthority(int authority)
        {
            String search = String.Format("select * from tb_user where authority={0}", authority);
            //dataoper.GetSqlDataset(search);
            return dataoper.GetSqlDataset(search);
        }
    }
